...
<Steps>
  ...
  <Step>
    ...
    The Session is already materialized: success commits the exchange, failure or cancellation leaves the transcript unchanged and can leave it empty. Mapping-write failures error the relay and reach the application's error reporter instead of being discarded. Later calls load the server-owned mapping and pass its `sessionId` to `client.chat()`.
